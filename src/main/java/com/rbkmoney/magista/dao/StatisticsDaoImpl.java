...
public class StatisticsDaoImpl extends NamedParameterJdbcDaoSupport implements StatisticsDao {
    ...
    public Collection<Map<String, String>> getAccountingDataByPeriod(Instant fromTime, Instant toTime) throws DaoException {
        String sql = "SELECT part1.merchant_id, part1.shop_id, part1.currency_code, funds_acquired, fee_charged, coalesce(opening_balance, 0) AS opening_balance, (coalesce(opening_balance, 0) + funds_acquired - fee_charged) AS closing_balance FROM (SELECT merchant_id, shop_id, invoice_currency_code AS currency_code, sum(payment_amount) AS funds_acquired, sum(Payment_fee) AS fee_charged FROM mst.invoice_event t1 WHERE event_id = (SELECT max(event_id) FROM mst.invoice_event t2 WHERE t2.invoice_id = t1.invoice_id AND t2.payment_id = t1.payment_id) AND payment_status = :succeeded_status::INVOICE_PAYMENT_STATUS AND payment_created_at >= :from_time AND payment_created_at < :to_time GROUP BY shop_id, invoice_currency_code, merchant_id) part1 LEFT JOIN (SELECT merchant_id, shop_id, sum(payment_amount - payment_fee) AS opening_balance FROM mst.invoice_event t1 WHERE event_id = (SELECT max(event_id) FROM mst.invoice_event t2 WHERE t2.invoice_id = t1.invoice_id AND t2.payment_id = t1.payment_id) AND payment_status = :succeeded_status::INVOICE_PAYMENT_STATUS AND payment_created_at < :from_time GROUP BY shop_id, merchant_id) part2 ON part1.merchant_id = part2.merchant_id AND part1.shop_id = part2.shop_id ORDER BY merchant_id, shop_id";
        MapSqlParameterSource params = new MapSqlParameterSource()
